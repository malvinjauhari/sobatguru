...
async function startServer() {
  ...
  app.post('/api/generate', async (req, res) => {
    try {
      const { document_type, context_statis, context_dinamis, context_warisan } = req.body;
      ...
      try {
        ...
        return res.status(500).json({ error: "AI menghasilkan format yang tidak valid. Silakan coba lagi." });
      }


      // Anti-Leakage Scrubbing: hapus sitasi di field tertentu
      if (document_type === 'MODUL_AJAR') {
         const scrub = (str) => {
            if (typeof str !== 'string') return str;
            // Hapus referensi ke "Sumber: ..." 
            str = str.replace(/sumber\s*:\s*[A-Za-z0-9\.\s\-&,\(\)]+/gi, '');
            // Hapus referensi URL yang bukan dari input
            // Kita bisa lebih agresif, tapi ini membatasi URL
            return str;
         };
         
         if (jsonData.pertemuan) {
            jsonData.pertemuan = jsonData.pertemuan.map((pt) => ({
               ...pt,
         ...
         if (jsonData.kisi_kisi_soal) {
             ...
             jsonData.kisi_kisi_soal = jsonData.kisi_kisi_soal.map((ks) => ({
                 ...ks,
                 teks_cp: cpText // Inject dari backend
             }));
         }
      }
      ...
      res.status(500).json({ error: error.message || 'Gagal berkomunikasi dengan AI Engine' });
    }
  });

  // API Route for Docx Export
  app.post('/api/export', async (req, res) => {
    try {
      // In a real application, you would load a template based on the type
      // e.g. const templateContent = fs.readFileSync(path.resolve(__dirname, 'templates/atp.docx'));
      // Then use docxtemplater to inject the variables, generate a buffer, and send it.
      
      // For this implementation, we will mock the docx generation and return a simple success
      // since providing real binary templates is not feasible directly via code blocks.
      
      res.json({ success: true, message: 'Document structure generated successfully.' });
    } catch (error) {
